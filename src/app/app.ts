...
const PYUSD_CONTRACT_ADDRESS = "0x0165878A594ca255338adfa4d48449f69242Eb8F"; // SimplePYUSD local
const VAULT_CONTRACT_ADDRESS = "0x2279B7A0a67DB372996a5FaB50D91eAA73d2eBe6"; // SimpleVault local
const LOAN_CONTRACT_ADDRESS = "0xa513E6E4b8f2a923D98304ec87F64353C4D5C853";  // SimpleLoan local
const BACKEND_API_URL = "http://localhost:8080/api/v1"; // URL del backend de Go
const LOCAL_RPC_URL = "http://127.0.0.1:8545"; // Red local de Hardhat
...
const VAULT_ABI = [
  ...
  // --- TAREA: Añadir funciones para gestionar guardianes ---
  // "function addGuardian(address _guardian)",
  // "function removeGuardian(address _guardian)",
  // "function getGuardians(address _user) view returns (address[])"
];

// ABI para LoanContract
const LOAN_ABI = [
  "function withdrawEmergencyLoan()"
  // --- TAREA: Añadir función para préstamo social ---
  // "function requestSocialCreditLoan(uint256 _amount)"
];
    ...
    @if (activeModal() === 'PROFILE') {
      <div class="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
        <div class="bg-gray-800 border border-cyan-500 text-white p-8 rounded-lg max-w-md w-full">
            ...
            <div class="space-y-4">
                <div><p class="text-sm text-gray-400">Nombre</p><p>Alex Castillo</p></div>
                <div><p class="text-sm text-gray-400">Email</p><p>alex.c@email.com</p></div>
...
export class App implements OnDestroy {
  ...
  async connectWallet() {
    ...
    try {
      if (typeof window.ethereum !== 'undefined') {
        ...
        authProvider = browserProvider;
        
        const accounts = await (window.ethereum as any).request({ method: 'eth_requestAccounts' });
        authSigner = await browserProvider.getSigner();
        authAddress = await authSigner.getAddress();
        
      } else {
        console.warn("MetaMask no detectado. Creando un signer de desarrollo (dummy).");
        // Para demo: usar red local con fork de mainnet
        console.log("Conectando a red local (fork de mainnet)...");
        authProvider = new ethers.JsonRpcProvider(LOCAL_RPC_URL);
        
        // Crea una wallet aleatoria - no tendrá fondos
        const dummyWallet = ethers.Wallet.createRandom();
        authSigner = dummyWallet.connect(authProvider);
        authAddress = await authSigner.getAddress();
        
        console.log("Wallet de desarrollo creada:", authAddress);
      }

      // 3. Guarda los datos del signer y cambia la vista
      this.provider.set(authProvider);
      this.signer.set(authSigner);
      this.signerAddress.set(authAddress);
      
      await this.loadRealWalletBalances();
      // --- TAREA: Aquí también cargar guardianes y estado de recuperación ---
      // await this.loadRealGuardianData(); 
      ...
      this.showError("No se pudo conectar la wallet. Revisa la consola para más detalles.");
      this.isLoading.set(false);
      return false;
    }
  }
  ...
  async loadRealWalletBalances() {
    ...
    try {
      ...
      // 3. Actualizar el signal de la wallet
      this.wallet.update(w => ({
        ...w,
        eth: parseFloat(formatUnits(ethBalance, 18)),
        pyusd: parseFloat(formatUnits(pyusdBalance, 18)) // SimplePYUSD tiene 18 decimales
      }));
      console.log("Balances reales cargados.");

    } catch (error) {
      console.error("Error cargando balances:", error);
      this.showError("No se pudieron cargar los balances. ¿Las direcciones de contrato (PYUSD) son correctas y están en la red Sepolia?");
    }
  }
  ...
  async addGuardian(name: string, contact: string) {
    ...
    this.isLoading.set(true);
    try {
      // TAREA: Llamar a tu contrato aquí
      ...
      // --- Simulación (borrar después de implementar lo de arriba) ---
      ...
      const newGuardian: Guardian = {
        ...
        contact,
        status: 'pending',
        avatarUrl: `https://placehold.co/40x40/gray/ffffff?text=${name.charAt(0).toUpperCase()}`
      };
      this.guardians.update(current => [...current, newGuardian]);
      // --- Fin de la simulación ---
      
      // En un caso real, se debería volver a cargar los guardianes desde el contrato:
      // await this.loadRealGuardianData();

      this.showError("Simulación: Guardián añadido localmente. Debes implementar la llamada al contrato.");

    } catch (error) {
      console.error("Error al añadir guardián:", error);
      this.showError("Error al añadir guardián. Revisa la consola.");
    }
    this.isLoading.set(false);
  }
  ...
  async upgradeToPremium() {
    ...
    this.isLoading.set(true);
    try {
      // 1. Crea una instancia del contrato PYUSD
      const pyusdContract = new Contract(PYUSD_CONTRACT_ADDRESS, ERC20_ABI, this.signer());
      
      // 2. Convierte 9.99 a unidades PYUSD (18 decimales)
      const amountWei = parseUnits("9.99", 18);
      
      // 3. Define la dirección del "Tesoro" del proyecto
      const treasuryAddress = "0x...DIRECCION_DE_TU_TESORERIA...";
      
      // 4. Llama a la función 'transfer' del contrato
      const tx = await pyusdContract['transfer'](treasuryAddress, amountWei);
      await tx.wait();
      
      // 5. Si tiene éxito, actualiza el estado local
      this.wallet.update(w => ({...w, pyusd: w.pyusd - 9.99}));
      const premiumTx: Transaction = {
        id: `tx${crypto.randomUUID()}`, txId: tx.hash, category: 'Pago', asset: 'PYUSD', amount: 9.99, usdValue: 9.99, date: new Date().toISOString().split('T')[0], walletSource: 'Guardian Vault'
      };
      ...
      console.error("Error al pagar la subscripción:", error);
      this.showError("Error al procesar el pago. Revisa si tienes suficientes fondos (PYUSD y ETH para gas).");
    }
    this.isLoading.set(false);
  }
  ...
  async startRecovery() { 
    ...
    // Pedir al usuario la *nueva* dirección
    const newOwnerAddress = prompt("Por favor, introduce la NUEVA dirección de wallet que tomará posesión de esta bóveda (la dirección de tu nueva wallet).");
    ...
    this.isLoading.set(true);
    try {
      // 1. Crea la instancia del contrato
      const vaultContract = new Contract(VAULT_CONTRACT_ADDRESS, VAULT_ABI, this.signer());
      
      // 2. Llama a la función 'startRecovery' del contrato
      ...
      this.showError("Error al iniciar la recuperación. ¿Estás seguro de que esta wallet está conectada a la bóveda correcta? Revisa la consola.");
    }
    this.isLoading.set(false);
  }
  ...
  async approveRecovery() { 
    ...
    this.isLoading.set(true);
    try {
      // 1. Crear la instancia del contrato
      const vaultContract = new Contract(VAULT_CONTRACT_ADDRESS, VAULT_ABI, this.signer());

      // 2. Llamar a la función 'approveRecovery' del contrato
      const tx = await vaultContract['approveRecovery']();
      await tx.wait();

      // 3. Actualizar el estado local (En un escenario real, deberías leer el estado desde el contrato)
      this.recoveryState.set('TIMELOCK'); 
      this.startTimelock();

    } catch (error) {
      console.error("Error al aprobar la recuperación:", error);
      this.showError("Error al aprobar la recuperación. ¿Eres un guardián válido? Revisa la consola.");
    }
    this.isLoading.set(false);
  }
  ...
  async useEmergencyCredit(amountStr: string) { 
    ...
    this.isLoading.set(true);
    try {
      // 1. Crear la instancia del contrato de Préstamo
      const loanContract = new Contract(LOAN_CONTRACT_ADDRESS, LOAN_ABI, this.signer());
      
      // 2. Llama a la función 'withdrawEmergencyLoan'
      // (Nota: El contrato real debería tomar un 'amount')
      const tx = await loanContract['withdrawEmergencyLoan'](); 
      await tx.wait();
      ...
      const loanTx: Transaction = {
          id: `tx${crypto.randomUUID()}`, txId: tx.hash, category: 'Préstamo', asset: 'PYUSD', amount: amount, usdValue: amount, date: new Date().toISOString().split('T')[0], walletSource: 'Guardian Vault'
      };
      ...
      console.error("Error al retirar el préstamo de emergencia:", error);
      this.showError("Error al retirar el préstamo. ¿El timelock está activo? Revisa la consola.");
    }
    this.isLoading.set(false);
  }
  ...
  async requestSocialCreditLoan(amountStr: string) {
    // !! TAREA: Esto debería implementarse de forma similar a 'useEmergencyCredit'
    // Necesitarías una función en tu contrato - ej. 'requestSocialLoan'
    // y probablemente un sistema de aprobación de guardianes.
    
    // Por ahora, queda como una simulación local:
    this.isLoading.set(true);
    await new Promise(resolve => setTimeout(resolve, 1000)); // Simula espera de red
    const amount = parseFloat(amountStr);
    if (!amount || amount <= 0) { return this.showError("Por favor, ingresa un monto válido."); }
    ...
    this.wallet.update(w => ({ ...w, pyusd: w.pyusd + amount }));
    const loanTx: Transaction = {
          id: `tx${crypto.randomUUID()}`, txId: `0xsocial...${crypto.randomUUID().substring(0, 4)}`, category: 'Préstamo', asset: 'PYUSD', amount: amount, usdValue: amount, date: new Date().toISOString().split('T')[0], walletSource: 'Guardian Vault'
      };
    this.internalTransactions.update(txs => [loanTx, ...txs]);
    this.activeModal.set(null);
    this.isLoading.set(false);
  ...
  verifyDeFiTransaction(address: string, amount: string) {
    ...
    this.http.post<SimulationResponse>(`${BACKEND_API_URL}/simulate_tx`, request)
      .subscribe({
        next: (response) => {

          this.defiTxRisk.set(response);
          this.defiTxStep.set('VERIFIED');
          this.isVerifyingTx.set(false);
        },
        error: (err) => {
          console.error("Error al verificar la transacción:", err);
          this.showError("Error al conectar con el Asistente de IA. ¿Está el backend de Go ejecutándose?");
          this.isVerifyingTx.set(false);
        }
      });
  }
  ...
  async executeDeFiTransaction() {
    ...
    this.isLoading.set(true);
    try {
      // 1. Crear instancia del contrato PYUSD
      const pyusdContract = new Contract(PYUSD_CONTRACT_ADDRESS, ERC20_ABI, this.signer());
      
      // 2. Convertir monto a unidades PYUSD (18 decimales)
      ...
      // (En una app 100% real, volvemos a llamar a loadRealWalletBalances()
      // en lugar de solo restar el valor)
      this.wallet.update(w => ({ ...w, pyusd: w.pyusd - amountNum }));
      
      const newTransaction: Transaction = {
        id: `tx${crypto.randomUUID()}`, txId: tx.hash, category: 'Pago', asset: 'PYUSD', amount: amountNum, usdValue: amountNum, date: new Date().toISOString().split('T')[0], walletSource: 'Guardian Vault'
      };
      this.internalTransactions.update(txs => [newTransaction, ...txs]);
      this.closeDeFiModal();

    } catch (error) {
      console.error("Error al ejecutar la transacción DeFi:", error);
      this.showError("Error al procesar el pago. Revisa la consola.");
    }
    this.isLoading.set(false);
  }
